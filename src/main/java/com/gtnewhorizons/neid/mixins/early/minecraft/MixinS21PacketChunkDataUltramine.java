 ...
 *
 * Priority 2000 ensures @Overwrite applies BEFORE base NEID Redirectors (priority 1000), preventing InjectionError when
 * Redirectors try to find calls in overwritten method.
 */
@Mixin(value = S21PacketChunkData.class, priority = 2000)
public abstract class MixinS21PacketChunkDataUltramine {
     ...
     *
     * CRITICAL: Reads from NEID arrays (block16BArray/block16BMetaArray), NOT from MemSlot! MemSlot only stores 12-bit
     * block IDs and 4-bit metadata, which truncates extended IDs/metadata.
     */
